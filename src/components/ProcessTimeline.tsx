    ...
    <section id="proceso" className="py-24 md:py-36 bg-background grain-overlay relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <SectionHeader
          ...
          titleClassName="font-serif text-5xl md:text-[4.2rem] lg:text-[5.6rem] font-bold mb-5 leading-tight"
        />
        ...
        {/* DESKTOP */}
        <div className="hidden md:block relative max-w-[62rem] mx-auto px-2 lg:px-4">
            ...
            {steps.map((step, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`flex items-center gap-4 lg:gap-6 py-8 will-change-transform ${isLeft ? "flex-row" : "flex-row-reverse"}`}
                >
                  <div className={`flex-1 group relative bg-card border border-border/60 rounded-xl p-11 lg:p-14 min-h-[295px] lg:min-h-[335px] flex flex-col justify-center hover:border-primary/30 transition-all duration-500 hover:shadow-glow overflow-hidden ${isLeft ? "text-right" : "text-left"}`}>
                    ...
                    <div className={`flex items-center gap-5 mb-5 ${isLeft ? "flex-row-reverse justify-start" : ""}`}>
                      <span className="text-border/50 font-serif text-5xl lg:text-[4.2rem] font-bold group-hover:text-primary/30 transition-colors duration-500 select-none">
                        {String(i + 1).padStart(2, "0")}
                      </span>
                      <div className="w-14 h-14 rounded-xl bg-primary/8 border border-primary/15 flex items-center justify-center group-hover:border-primary/40 group-hover:bg-primary/12 transition-all duration-500">
                        <step.icon className="w-6 h-6 text-primary" strokeWidth={1.5} />
                      </div>
                    </div>
                    <h3 className="font-serif text-2xl lg:text-[2.1rem] font-semibold mb-3.5 group-hover:text-primary transition-colors duration-400">{step.title}</h3>
                    <p className="text-muted-foreground text-[1.05rem] mb-6 leading-relaxed">{step.desc}</p>
                    <span className="inline-block text-xs text-primary bg-primary/8 border border-primary/15 px-4 py-2 rounded-full font-medium tracking-wider uppercase">
                      {step.time}
                    </span>
                  </div>
